...
export default function MessageList() {
  ...
  // Filter messages for current room - memoized
  const roomMessages = useMemo(
    () => messages.filter((msg) => msg.roomId === currentRoom?.id),
    [messages, currentRoom?.id]
  )

  // Memoize message data to prevent recalculating timestamps on every render
  const processedMessages = useMemo(() => {
    return roomMessages.map((message, index) => {
      ...
      const showAvatar =
        ...
        messageTime - prevMessageTime > 60000 // 1 minute

      const showTimestamp =
        index === 0 ||
        messageTime - prevMessageTime > 300000 // 5 minutes
      ...
      return {
        ...
        showAvatar,
        showTimestamp,
        formattedTimestamp: format(new Date(message.timestamp), 'PPp', { locale: zhTW }),
  ...
  return (
    ...
    >
      {roomMessages.length === 0 ? (
        ...
        <>
          {processedMessages.map((props) => (
            <MessageItem key={props.message.id} {...props} />
          ))}
