...
describe("MultiSigWallet", function () {
  ...
  beforeEach(async function () {
    ...
    wallet = await MultiSig.deploy(
      ...
      ethers.parseEther("1") // daily limit: 1 ETH
    );
  ...
  describe("Deployment", function () {
    ...
    it("Should have correct balance", async function () {
      expect(await wallet.getBalance()).to.equal(ethers.parseEther("10"));
    });
  });
  ...
  describe("Transactions", function () {
    ...
    it("Should approve and execute transaction", async function () {
      await wallet.submitTransaction(addr1.address, ethers.parseEther("0.5"), "0x");
      ...
      expect(balanceAfter.sub(balanceBefore)).to.equal(ethers.parseEther("0.5"));
    });
    ...
    it("Should respect daily limit", async function () {
      await wallet.submitTransaction(addr1.address, ethers.parseEther("2"), "0x");
      await wallet.connect(owner1).approveTransaction(0);
      await wallet.connect(owner2).approveTransaction(0);
